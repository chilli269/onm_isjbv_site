...
class DeliCloudPwaSettingData(models.Model):
    """
    """
    _name = "delicloud_pwa.setting_data"
    _description = "Delicloud PWA Setting Data"
    ...
    def _get_default_icon(self, size=512):
        ...
        tmp_path = get_resource_path(
            'delicloud_pwa', 'static', 'icons', 'icon-{size}x{size}.png'.format(size=size))
    ...
    def get_system_config(self):
        ...
        :return:
        """
        record = self.env["delicloud_pwa.setting_data"].search(
            [("company_id", "=", False)])
        if not record:
