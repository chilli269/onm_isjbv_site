...
class DeliCloudPwaSetting(models.TransientModel):
    """
    """
    ...
    @api.model
    def get_values(self):
        res = super(DeliCloudPwaSetting, self).get_values()

        config = self.get_delicloud_pwa_config_record()
    ...
    def get_delicloud_pwa_config_record(self):
        """
        get config
        """
        # get type form ir.config_parameter
        pwa_type = self.env["ir.config_parameter"].sudo().get_param('delicloud_pwa.pwa_type', 'system')
        if pwa_type == 'system':
            return self.env["delicloud_pwa.setting_data"].get_system_config()
        else:
            return self.env["delicloud_pwa.setting_data"].get_company_config()

    def get_delicloud_pwa_config(self):
        """
        ...
        if config.pwa_icon_128:
            values["pwa_icon_128"] = '/web/image/delicloud_pwa.setting_data/{var_id}/pwa_icon_128'.format(var_id=config.id)
        else:
            # the static image
            values["pwa_icon_128"] = '/delicloud_pwa/static/src/img/icon128x128.png'

        if config.pwa_icon_144:
            values["pwa_icon_144"] = '/web/image/delicloud_pwa.setting_data/{var_id}/pwa_icon_144'.format(var_id=config.id)
        else:
            # the static image
            values["pwa_icon_144"] = '/delicloud_pwa/static/src/img/icon144x144.png'

        if config.pwa_icon_152:
            values["pwa_icon_152"] = '/web/image/delicloud_pwa.setting_data/{var_id}/pwa_icon_152'.format(
                var_id=config.id)
        else:
            # the static image
            values["pwa_icon_152"] = '/delicloud_pwa/static/src/img/icon152x152.png'

        if config.pwa_icon_192:
            values["pwa_icon_192"] = '/web/image/delicloud_pwa.setting_data/{var_id}/pwa_icon_192'.format(
                var_id=config.id)
        else:
            # the static image
            values["pwa_icon_192"] = '/delicloud_pwa/static/src/img/icon192x192.png'

        if config.pwa_icon_256:
            values["pwa_icon_256"] = '/web/image/delicloud_pwa.setting_data/{var_id}/pwa_icon_256'.format(
                var_id=config.id)
        else:
            # the static image
            values["pwa_icon_256"] = '/delicloud_pwa/static/src/img/icon256x256.png'

        if config.pwa_icon_512:
            values["pwa_icon_512"] = '/web/image/delicloud_pwa.setting_data/{var_id}/pwa_icon_512'.format(
                var_id=config.id)
        else:
            # the static image
            values["pwa_icon_512"] = '/delicloud_pwa/static/src/img/icon512x512.png'
    ...
    def set_values(self):
        ...
        Set values
        """
        res = super(DeliCloudPwaSetting, self).set_values()
        
        # save enable pwa and pwa type to ir.config_parameter
        self.env["ir.config_parameter"].sudo().set_param('delicloud_pwa.enable_pwa', self.enable_pwa)
        self.env["ir.config_parameter"].sudo().set_param('delicloud_pwa.pwa_type', self.pwa_type)
    ...
    def is_pwa_enabled(self):
        ...
        Is PWA enabled
        """
        enable_pwa = self.env["ir.config_parameter"].sudo().get_param('delicloud_pwa.enable_pwa', True)
        return enable_pwa
