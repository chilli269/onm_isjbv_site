...
class PWA(Controller):
    ...
    @route("/delicloud-service-worker.js", type="http", auth="public")
    def service_worker(self):
        ...
        content = qweb._render(
            "delicloud_pwa.service_worker", {
                "delicloud_pwa_cache_name": cache_version,
                "delicloud_pwa_files_to_cache": urls
            },
        )
        return request.make_response(content, [("Content-Type", mimetype)])

    @route("/delicloud_pwa/manifest", type="http", auth="public")
    def manifest(self):
        '''
        ...
        pwa_config = request.env["res.config.settings"].get_delicloud_pwa_config()
        ...
        content = qweb._render("delicloud_pwa.manifest", {
            "pwa_name": pwa_name,
